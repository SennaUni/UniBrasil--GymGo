...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    ...
    if ref_landmarks is None:
        ...
        while cap_ref.isOpened():
            ...
            landmarks_ref = np.array([[lm.x, lm.y] for lm in results_ref.pose_landmarks.landmark])
    ...
    while cap_comp.isOpened():
        ...
        # Converter os pontos de referência e pontos de comparação para arrays 2D
        ref_landmarks_2d = np.expand_dims(ref_landmarks[0], axis=0)
        landmarks_comp_2d = np.expand_dims(np.array([[lm.x, lm.y] for lm in results_comp.pose_landmarks.landmark]), axis=0)

        # Calcular a matriz de transformação perspectiva entre os pontos de referência no vídeo de referência e comparação
        transform_matrix, _ = cv2.findHomography(ref_landmarks_2d, landmarks_comp_2d)

        # Aplicar a transformação perspectiva nos pontos capturados do vídeo de comparação
        landmarks_comp_transformed = cv2.perspectiveTransform(landmarks_comp_2d, transform_matrix)

        # Comparar os movimentos do vídeo de comparação transformado com os do vídeo de referência
        similarity_score = np.mean(np.abs(landmarks_comp_transformed[:, :, :2] - ref_landmarks_2d[:, :, :2]))
