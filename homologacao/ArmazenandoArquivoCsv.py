...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    ...
    try:
        ref_landmarks = pd.read_csv(landmarks_file, header=None).values
        ref_landmarks = [[float(val) for val in row[0].split(',')] for row in ref_landmarks]
    ...
    if ref_landmarks is None:
        ...
        ref_landmarks_df = pd.DataFrame(ref_landmarks)
        ref_landmarks_df.to_csv(landmarks_file, header=False, index=False)

    # Converter os dados de referência para uma lista de arrays numpy
    ref_landmarks = [np.array(landmarks) for landmarks in ref_landmarks]
    ...
    while cap_comp.isOpened():
        ...
        similarity_scores = [np.mean(np.abs(landmarks_comp - landmarks_ref)) for landmarks_ref in ref_landmarks]
