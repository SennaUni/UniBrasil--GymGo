...
with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    ...
    exercises_count = 0
    exercise_started = False  # Set to True initially
    current_index = 0
    progress = [False, False, False]  # Inicialmente nenhum movimento foi detectado
    ...
    while cap_comp.isOpened():
        ...
        if not exercise_started:
            ...
            if similarity_score <= similarity_threshold:
                ...
                if current_index == len(ref_landmarks_points):
                    current_index = 0
                    exercises_count += 1

                    # Atualizar o progresso
                    progress[current_index] = True

                    # Verificar se todos os movimentos foram concluídos
                    if all(progress):
                        progress = [False, False, False]  # Redefinir o progresso
                        exercises_count = 0  # Reiniciar a contagem
        ...
        cv2.putText(annotated_image_comp, f'Exercises: {exercises_count}', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1,
                    (0, 0, 255), 2)
